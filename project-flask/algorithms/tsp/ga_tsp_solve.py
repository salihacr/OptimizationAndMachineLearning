...
class GA(object):
    ...
    def cross(self, parent1, parent2):
        ...
        while 1:
            new_genes = []
            index1 = random.randint(0, self.gene_length - 1)  #Belirlenen aralıkta rastgele tamsayı oluştur.
            ...
            for g in parent1.gene:
                if len_parent1 == index1:
                    new_genes.extend(tempGene)  #Yeni gene çaprazlanan parçaları ekle.
                    len_parent1 += 1
    ...
    def mutation(self, egg):
        ...
        new_genes = egg.gene[:]  #Mutasyon sırasında ana popülasyonu etkilememek için yeni bir gen dizisi oluşturulur.
    ...
    def next(self):
        ...
        self.judge()
        new_lives = []
        new_lives.append(self.best_genes)  #En iyi bireyler korunarak gelecek nesillere aktarılır.
...
class GA_TSP_SOLVE(object):
    ...
    def plot_cities(self, x_axis, y_axis, best_route,best_cost):
        ...
        for i in range(len(self.cities)):
            ax.annotate(self.cities[i] , xy = get_XY_location(self,x_axis, y_axis, i), c = "black")
        
        plt.plot(x_axis[path], y_axis[path], c = "purple")
        plt.show()
